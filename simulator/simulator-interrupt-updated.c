...
int decode(uint32_t instr, uint8_t * p_opcode, uint8_t * p_sreg, uint8_t * p_treg, int8_t * p_imm)
{
	...
	*p_opcode = *(strp + 3);
	...
	/*
	if (*p_imm >= MAX_MEM_SIZE || *p_opcode > 0x11 || *p_opcode < 0)
	{
		return -1;
	}*/
	return 0;
...
{	
	...
	switch (*p_opcode){
		...
		case OP_PUSH:
			cpu->SP -= 1;
			memory->addr[cpu->SP] = cpu->R[*p_sreg];
			cpu->PC++;
			break;
		case OP_POP:
			cpu->R[*p_treg] = memory->addr[cpu->SP];
			cpu->SP++;
			cpu->PC++;
			break;
		case OP_IRET:
			cpu->PC = memory->addr[cpu->SP];
			cpu->SP++;
			cpu->PSR = memory->addr[cpu->SP];
			cpu->SP++;
			break;
		case OP_PUT:
			printf("%c", cpu->R[*p_sreg]);
			cpu->PC++;
			break;
		default:
			printf("here????\n");
...
int check_interrupt(COMPUTER* cp)
{
	//If the interrupt enable bit and the interrupt pending bit are both one,
	    //       1. Clear up the interrupt pending bit (=0);
        //       2. Save PSR and PC onto the stack by using the code below:
        //			cp->cpu.SP -= 1;
...
int computer_load_init(COMPUTER * cp, char * file)
{
    ...
    cp->cpu.PC=0;	//Program counter,the memory address starts from 0
    cp->cpu.IR=0; 	//Instruction regiser, one instruction is 4B
